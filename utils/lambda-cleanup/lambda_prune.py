...
def main():
    ...
    [
        prune_function(lambda_client, function)
...
def prune_function(lambda_client, function):
    ...
    [
        remove_version(lambda_client, arn)
        for arn in old_versions(lambda_client, function)
    ]
...
def old_versions(lambda_client, function):
    ...
    return [
        v["FunctionArn"]
        for v in [
            version for version in versions if version["Version"] != function["Version"]
        ]
    ]
